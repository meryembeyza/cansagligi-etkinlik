...
export default function CalendarView({ userRole, userRegion, userId }: { userRole: string, userRegion: string, userId?: string }) {
    ...
    days.push(
      <div key={`day-${i}`} className={`calendar-day ${isToday ? 'today' : ''}`}>
        ...
        <div className="day-events">
          {dayEvents.map(ev => {
            const color = UNIT_COLORS[ev.event_type] || UNIT_COLORS['Diğer'];
            return (
              <div 
                key={ev.id} 
                ...
                title={ev.event_name}
              >
                <span className="event-time">{new Date(ev.event_date).toLocaleTimeString('tr-TR', {hour: '2-digit', minute:'2-digit'})}</span>
                <span className="event-title">{ev.event_name}</span>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div className="calendar-container">
      ...
      {/* Sol Menü (Lejant ve Filtreler) - Birim Başkanı için gizli */}
      {userRole !== 'unit_head' && (
