...
export default function PublishToBursaryModal({ event, onClose, onSuccess }: PublishToBursaryModalProps) {
  ...
  const validateAndShowPreview = () => {
    if (!formData.display_title || !formData.event_date || !formData.city) {
      toast.error("Lütfen zorunlu alanları (Etkinlik Adı, Tarih ve Şehir) doldurunuz.");
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (posterFile) {
        ...
        if (uploadError) throw new Error("Afiş yüklenemedi. Lütfen 'posters' adında bir storage bucket olduğundan emin olun. Detay: " + upload(error as Error).message);
      ...
      if (insertError) {
        throw new Error("Veritabanına ekleme hatası. Tablo oluşturulmamış veya yetki sorunu olabilir. Detay: " + insert(error as Error).message);
      }
