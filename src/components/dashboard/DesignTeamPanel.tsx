...
export default function DesignTeamPanel() {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      console.error('Afiş talepleri çekilemedi:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleFileUpload = async (requestId: string, eventId: string, eventCreatorId: string, eventName: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (file.size > 15 * 1024 * 1024) {
        throw new Error('Dosya boyutu 15MB limitini açıyor.');
      }
      ...
      await supabase.from('notifications').insert([{
        ...
        event_id: eventId,
        message: `Tasarım ekibi "${eventName}" etkinliği için afiş dosyasını yükledi.`,
        type: 'poster_update'
      }]);

      await fetchRequests();
    } catch (err) {
      toast.error('Dosya yüklenirken hata oluştu: ' + (err as Error).message);
    } finally {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #eaeaea', textAlign: 'left' }}>
              <th style={{ padding: '1rem 0.75rem' }}>Etkinlik Adı</th>
              <th style={{ padding: '1rem 0.75rem' }}>Tarih</th>
              <th style={{ padding: '1rem 0.75rem' }}>Durum</th>
              <th style={{ padding: '1rem 0.75rem' }}>İşlem</th>
            </tr>
          </thead>
          <tbody>
            {requests.map((req) => (
              <tr key={req.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                <td style={{ padding: '1rem 0.75rem' }}>
                  <div style={{ fontWeight: 600, color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    ...
                    >
                      {req.events?.event_name || 'Silinmiş Etkinlik'}
                    </button>
                  </div>
                  {req.designer_notes && (
                ...
                <td style={{ padding: '1rem 0.75rem' }}>
                  <span className={`badge ${
                    ...
                    req.status === 'Hazırlanıyor' ? 'badge-warning' :
                ...
                <td style={{ padding: '1rem 0.75rem' }}>
                  {req.status === 'Bekliyor' && (
                    ...
                    >
                      {processingId === req.id ? <Loader2 size={14} className="animate-spin" /> : <Clock size={14} style={{ marginRight: '4px', display: 'inline' }}/>} İşleme Al
                    </button>
                  )}

                  {(req.status === 'Hazırlanıyor' || req.status === 'Revizyon Gerekli') && (
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div style={{ backgroundColor: 'var(--bg-card)', width: '100%', maxWidth: '550px', borderRadius: 'var(--radius-lg)', padding: '2rem', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', fontSize: '0.9rem', color: 'var(--text-main)', marginBottom: '1.5rem' }}>
              ...
              <div>
                <strong style={{ color: 'var(--text-muted)' }}>Afişte Bulunması Gereken Logolar:</strong>
                <div style={{ padding: '0.75rem', backgroundColor: 'var(--bg-info-light)', border: '1px solid var(--border-info)', borderRadius: '4px', marginTop: '0.25rem', whiteSpace: 'pre-wrap', fontWeight: 500 }}>
                  {selectedRequest.required_logos || 'Belirtilmemiş.'}
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ marginTop: '0.25rem' }}>
                  <span className={`badge ${
                    selectedRequest.events?.status === 'Onaylandı' ? 'badge-success' :
