...
export default function SettingsPage() {
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (passwordForm.newPassword !== passwordForm.confirmPassword) {
      toast.success('Yeni þifreler eþleþmiyor!');
      return;
    }
    ...
    try {
      ...
      toast.success('Åžifreniz baþarýyla güncellendi.');
  ...
  const handleSignOutOtherDevices = async () => {
    ...
    if (window.confirm("Bu cihaz haricindeki tüm oturumlarýnýzý sonlandýrmak istediðinize emin misiniz?")) {
      toast.success("Diðer cihazlardaki oturumlar baþarýyla sonlandýrýldý. (Simülasyon)");
    }
  };
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto', paddingBottom: '4rem' }}>
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <Settings size={24} color="var(--color-primary)" /> Hesap Ayarlarý
        </h1>
        <p style={{ color: 'var(--text-muted)' }}>Bildirimlerinizi, güvenlik tercihlerinizi ve þifrenizi buradan yönetebilirsiniz.</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ width: '250px', display: 'flex', flexDirection: 'column', gap: '0.5rem', flexShrink: 0 }}>
          ...
          <div 
            ...
            className="hover-bg-gray"
          >
            <Lock size={18} /> Åžifre ve Güvenlik
          </div>
        </div>
        ...
        <div style={{ flex: 1, minWidth: '300px' }}>
            ...
            <div className="card" style={{ animation: 'fadeIn 0.3s ease' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <h4 style={{ fontWeight: 600 }}>Uygulama Ýçi Bildirimler</h4>
                    <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>Etkinlik durumu deðiþiklikleri sað üstteki çan ikonuna düþsün.</p>
                  </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>Önemli uyarýlar kayýtlý e-posta adresinize gönderilsin.</p>
                  </div>
                  <label className="toggle-switch">
              ...
              <div style={{ marginTop: '2rem', display: 'flex', justifyContent: 'flex-end' }}>
                <button className="btn btn-primary" onClick={() => toast.success("E-posta bildirim ayarlarýnýz güncellendi.")}>Kaydet</button>
              </div>
            </div>
          )}
          ...
          {activeTab === 'security' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', animation: 'fadeIn 0.3s ease' }}>
              <div className="card">
                <h3 style={{ fontSize: '1.25rem', fontWeight: 600, marginBottom: '1.5rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>Åžifre Deðiþtir</h3>
                <form onSubmit={handlePasswordChange} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '400px' }}>
                  <div>
                    <label className="label">Mevcut Åžifreniz</label>
                  ...
                  <button type="submit" disabled={isChangingPassword} className="btn btn-primary" style={{ marginTop: '1rem' }}>
                    {isChangingPassword ? 'Deðiþtiriliyor...' : 'Åžifreyi Güncelle'}
                  </button>
                </form>
              </div>
              ...
              <div className="card" style={{ border: '1px solid #fca5a5', backgroundColor: 'var(--bg-danger-light)' }}>
                <h3 style={{ fontSize: '1.25rem', fontWeight: 600, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#991b1b' }}>
                  <Shield size={20} /> Oturum Güvenliði
                </h3>
                <p style={{ color: '#7f1d1d', fontSize: '0.875rem', marginBottom: '1rem' }}>
                  Eðer hesabýnýzýn baþka bir cihazda açýk kaldýðýný düþünüyorsanýz, mevcut cihazýnýz hariç diðer tüm oturumlarý sonlandýrabilirsiniz.
                </p>
                <button onClick={handleSignOutOtherDevices} className="btn" style={{ backgroundColor: 'var(--status-danger)', color: 'white' }}>
                  Diðer Cihazlardan Çýkýþ Yap
                </button>
              </div>
            </div>
          )}
