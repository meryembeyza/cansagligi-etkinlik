...
export function LegacyLogisticsTab() {
  ...
  const handleToggleCargo = async (eventId: string, currentCargoSent: boolean) => {
    try {
      const { data: event, error: fetchErr } = await supabase.from('events').select('budget_request').eq('id', eventId).single();
      if (fetchErr || !event) throw new Error('Etkinlik bulunamadı');

      const logData = JSON.parse(event.budget_request);
      logData.cargoSent = !currentCargoSent;

      const { error: updateErr } = await supabase.from('events').update({ budget_request: JSON.stringify(logData) }).eq('id', eventId);
      if (updateErr) throw new Error('Kargo durumu güncellenemedi');
    ...
    try {
      ...
      data?.forEach((event: any) => {
        try {
          const logData = JSON.parse(event.budget_request);
          if (!logData) return;
          const cargoSent = logData.cargoSent || false;
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
              <thead>
                <tr style={{ backgroundColor: 'var(--bg-main)', color: 'var(--text-muted)', textAlign: 'left' }}>
                  ...
                  <th style={{ padding: '0.75rem 1rem', borderBottom: '1px solid var(--border-color)', width: '30%' }}>Talep Detayları</th>
                  <th style={{ padding: '0.75rem 1rem', borderBottom: '1px solid var(--border-color)' }}>Etkinlik Durumu</th>
                  <th style={{ padding: '0.75rem 1rem', borderBottom: '1px solid var(--border-color)' }}>Kargo Durumu</th>
              ...
              <tbody>
                {filteredData.map((item, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid var(--border-color)' }}>
                    ...
                    <td style={{ padding: '1rem' }}>
                      {item.region === 'İstanbul Avrupa' || item.region === 'İstanbul Anadolu' ? (
                        <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Elden Teslim (İstanbul)</span>
                      ) : (
                        <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.85rem' }}>
                          <input 
                            type="checkbox" 
                            checked={item.cargoSent} 
                            onChange={() => handleToggleCargo(item.eventId, item.cargoSent)} 
                            style={{ width: '16px', height: '16px', accentColor: 'var(--color-primary)' }}
                          />
                          {item.cargoSent ? <span style={{ color: 'var(--color-primary)', fontWeight: 600 }}>Gönderildi</span> : <span style={{ color: 'var(--text-muted)' }}>Bekliyor</span>}
                        </label>
                      )}
                    </td>
