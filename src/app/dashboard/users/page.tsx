...
export default function UsersAdminPage() {
  ...
  const handleApprove = async (userId: string, userName: string) => {
    ...
    try {
      ...
      toast.error('Onaylama sırasında hata oluştu: ' + (err as Error).message);
    }
  };
  ...
  const handleReject = async (userId: string, userName: string) => {
    ...
    try {
      ...
      toast.error('Silme sırasında hata oluştu: ' + (err as Error).message);
    }
  };
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                <thead>
                  <tr style={{ backgroundColor: 'var(--bg-main)', color: 'var(--text-muted)', textAlign: 'left' }}>
                    ...
                    <th style={{ padding: '0.75rem 1rem', borderBottom: '1px solid var(--border-color)' }}>İletişim & tır. No</th>
                  ...
                  {pendingUsers.map(user => (
                    <tr key={user.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      ...
                      <td style={{ padding: '1rem' }}>
                        ...
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>tır No: {user.student_id || '-'}</div>
                      </td>
